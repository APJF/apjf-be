...
public class ApprovalRequestController {
    ...
    public ResponseEntity<ApiResponseDto<List<ApprovalRequestDto>>> getAll(
        ...
        log.info("Fetching approval requests with filters - pending: {}, targetType: {}, createdBy: {}, reviewedBy: {}",
                pending, targetType, createdBy, reviewedBy);

        List<ApprovalRequestDto> requests;
        String message = "Danh sách approval requests";

        if (pending != null && pending) {
            requests = approvalRequestService.findPending();
            message = "Danh sách approval requests chờ duyệt";
